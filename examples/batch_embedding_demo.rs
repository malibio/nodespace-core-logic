...
impl DataStore for BatchTestDataStore {
    ...
    async fn search_similar_nodes(&self, _embedding: Vec<f32>, limit: usize) -> NodeSpaceResult<Vec<(Node, f32)>> {
        ...
        let results: Vec<(Node, f32)> = nodes.values().take(limit).enumerate()
            .map(|(i, node)| (node.clone(), 1.0 - (i as f32 * 0.1))).collect();
        Ok(results)
    }
...
async fn main() -> NodeSpaceResult<()> {
    ...
    // Prepare test data
    let test_data = vec![
        ("This is the first test document about AI and machine learning".to_string(), json!({"type": "document", "category": "ai"})),
        ("Second document discusses database optimization techniques".to_string(), json!({"type": "document", "category": "database"})),
        ("Third text covers web development frameworks and patterns".to_string(), json!({"type": "document", "category": "web"})),
        ("Fourth piece explores cloud computing architectures".to_string(), json!({"type": "document", "category": "cloud"})),
        ("Fifth document analyzes cybersecurity best practices".to_string(), json!({"type": "document", "category": "security"})),
        ("Sixth text examines data science methodologies".to_string(), json!({"type": "document", "category": "data"})),
        ("Seventh document reviews mobile app development".to_string(), json!({"type": "document", "category": "mobile"})),
        ("Eighth piece discusses DevOps automation tools".to_string(), json!({"type": "document", "category": "devops"})),
        ("Ninth text covers blockchain technology concepts".to_string(), json!({"type": "document", "category": "blockchain"})),
        ("Tenth document explores quantum computing principles".to_string(), json!({"type": "document", "category": "quantum"})),
    ];
    ...
    for (content, metadata) in &test_data {
        let node_id = service.create_knowledge_node(content, metadata.clone()).await?;
        individual_node_ids.push(node_id);
    }
    
    let individual_duration = individual_start.elapsed();
    println!("✅ Individual processing: {:.2}ms ({:.1} nodes/sec)", 
    ...
    let batch_node_ids = service.create_knowledge_nodes_batch(test_data.clone()).await?;
    
    let batch_duration = batch_start.elapsed();
    println!("✅ Batch processing: {:.2}ms ({:.1} nodes/sec)", 
    ...
    println!("\n📈 Performance Results:");
    println!("   Individual: {:.2}ms total, {:.1} nodes/sec", 
             individual_duration.as_millis(),
             test_data.len() as f64 / individual_duration.as_secs_f64());
    println!("   Batch:      {:.2}ms total, {:.1} nodes/sec", 
             batch_duration.as_millis(),
             test_data.len() as f64 / batch_duration.as_secs_f64());
    ...
    // Verify results
    println!("\n✅ Verification:");
    println!("   Individual method created: {} nodes", individual_node_ids.len());
    println!("   Batch method created:      {} nodes", batch_node_ids.len());
    println!("   All node IDs are unique:   {}", 
             individual_node_ids.iter().chain(batch_node_ids.iter()).collect::<std::collections::HashSet<_>>().len() 
             == individual_node_ids.len() + batch_node_ids.len());

    println!("\n🎉 Batch Embedding Optimization Test Completed!");
    println!("   Target achieved: {} nodes/sec (target: 10+ nodes/sec)", 
             test_data.len() as f64 / batch_duration.as_secs_f64());
