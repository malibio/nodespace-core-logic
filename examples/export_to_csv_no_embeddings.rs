...
/// Export all database tables to CSV files (excluding vector embeddings)
#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("📊 Exporting Database to CSV Files (No Embeddings)\n");
    ...
    for table_name in &table_names {
        ...
        // Create CSV file with _no_embeddings suffix
        let csv_filename = format!("{}_no_embeddings.csv", table_name);
        let mut csv_file = File::create(&csv_filename)?;
        println!("   📄 Creating CSV file: {}", csv_filename);

        // Write CSV header (excluding vector column)
        let schema = &batches[0].schema();
        let field_names: Vec<String> = schema
            .fields()
            .iter()
            .filter(|field| field.name() != "vector") // Skip the vector/embedding column
        ...
        for (batch_idx, batch) in batches.iter().enumerate() {
            ...
            for row_idx in 0..batch.num_rows() {
                ...
                for col_idx in 0..batch.num_columns() {
                    ...
                    // Skip the vector/embedding column
    ...
    for table_name in &table_names {
        println!("   📄 {}_no_embeddings.csv", table_name);
    }
