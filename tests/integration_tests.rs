use nodespace_core_logic::{NodeSpaceService, CoreLogic, SearchResult, QueryResponse};
use nodespace_core_types::{Node, NodeId, NodeSpaceResult};
use nodespace_data_store::{DataStore, SurrealDataStore};
use nodespace_nlp_engine::{NLPEngine, LocalNLPEngine};
use serde_json::json;
use tokio_test;
...
impl NLPEngine for MockNLPEngine {
    ...
    async fn generate_surrealql(&self, _natural_query: &str, _schema_context: &str) -> NodeSpaceResult<String> {
...
async fn test_create_knowledge_node() {
    ...
    let node_id = service.create_knowledge_node("Test content", metadata).await.unwrap();
...
async fn test_semantic_search() {
    ...
    let _id1 = service.create_knowledge_node("First test document", metadata.clone()).await.unwrap();
    let _id2 = service.create_knowledge_node("Second test document", metadata).await.unwrap();
...
async fn test_process_query() {
    ...
    let _node_id = service.create_knowledge_node("NodeSpace is a knowledge management system", metadata).await.unwrap();
...
async fn test_update_node() {
    ...
    // Create a test node
    let metadata = json!({"type": "test"});
    let node_id = service.create_knowledge_node("Original content", metadata).await.unwrap();
...
async fn test_get_related_nodes() {
    ...
    // Create a test node
    let metadata = json!({"type": "test"});
    let node_id = service.create_knowledge_node("Test node with relationships", metadata).await.unwrap();
    
    // Get related nodes
    let related = service.get_related_nodes(&node_id, vec!["related".to_string()]).await.unwrap();
...
async fn test_generate_insights() {
    ...
    let id1 = service.create_knowledge_node("First insight content", metadata.clone()).await.unwrap();
    let id2 = service.create_knowledge_node("Second insight content", metadata).await.unwrap();
