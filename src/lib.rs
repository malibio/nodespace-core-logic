...
// Implement CoreLogic trait for NodeSpaceService for compatibility
#[async_trait]
impl<D: DataStore, N: NLPEngine> CoreLogic for NodeSpaceService<D, N> {
    ...
    async fn create_text_node(&self, content: &str, date: &str) -> NodeSpaceResult<NodeId> {
        // Generate embedding and create node
    ...
    async fn add_child_node(&self, parent_id: &NodeId, child_id: &NodeId) -> NodeSpaceResult<()> {
        self.data_store.create_relationship(parent_id, child_id, "contains").await
    }

    async fn get_child_nodes(&self, parent_id: &NodeId) -> NodeSpaceResult<Vec<Node>> {
        let query = format!("SELECT * FROM {}->contains", parent_id);
    ...
    async fn make_siblings(&self, left_node_id: &NodeId, right_node_id: &NodeId) -> NodeSpaceResult<()> {
        self.data_store.create_relationship(left_node_id, right_node_id, "next_sibling").await?;
        self.data_store.create_relationship(right_node_id, left_node_id, "previous_sibling").await?;
        Ok(())
    }
