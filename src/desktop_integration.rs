...
use crate::{NodeSpaceService, DataStore, NLPEngine, CoreLogic, HierarchyComputation};
use nodespace_core_types::{NodeId, NodeSpaceResult, NodeSpaceError};
...
impl<D: DataStore + Send + Sync, N: NLPEngine + Send + Sync> NodeSpaceService<D, N> {
    ...
    pub async fn upsert_node(
        ...
        log::info!("🔄 Upserting node: {} (type: {})", node_id.as_str(), node_type);
        ...
        if let Some(mut node) = existing_node {
            ...
            if let Some(ref before_sibling) = before_sibling_id {
                self.update_sibling_order(&node_id, Some(before_sibling), parent_id.as_ref()).await?;
            }
            
            // Regenerate embedding with new content
            let embedding = self.nlp_engine.generate_embedding(embedding_content).await?;
    ...
    pub async fn process_query_enhanced(&self, query: String) -> NodeSpaceResult<EnhancedQueryResponse> {
        ...
        let ai_response = match self.generate_ai_response(&query, &search_results.iter().map(|r| r.node.id.clone()).collect::<Vec<_>>()).await {
        ...
        let overall_confidence = if enhanced_sources.is_empty() {
            ...
            let avg_source_score = enhanced_sources.iter().map(|s| s.retrieval_score).sum::<f64>() / enhanced_sources.len() as f64;
            (avg_source_score * 0.8) + 0.2 // Weighted by source quality
        };

        log::info!("   ✅ Enhanced query processed in {}ms", generation_time);
        log::info!("   📊 Sources: {}, Confidence: {:.2}", enhanced_sources.len(), overall_confidence);
